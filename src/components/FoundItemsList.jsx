...
const FoundItemsList = () => {
  ...
  useEffect(() => {
    const q = query(collection(db, "found_items"), orderBy("date_reported", "desc"));
    ...
    <div>
      <div className="flex flex-col md:flex-row gap-4 mb-8">
        <div className="relative flex-grow">
          <input 
            ...
            className="w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-emerald-600/5 focus:border-emerald-600 transition-all font-medium text-sm"
          />
        ...
        <select 
          ...
          className="bg-white border border-slate-200 rounded-2xl px-6 py-3 text-sm font-bold outline-none focus:border-emerald-600 transition-colors"
        >
      ...
      {filteredItems.length === 0 ? (
        <div className="text-center py-20 text-slate-400 italic">
          {foundItems.length === 0 ? "No items have been found yet." : "No results for this search."}
        </div>
      ) : (
              ...
              >
                ...
                <div className="p-8 flex flex-col flex-1">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <span className="px-2 py-0.5 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded text-[10px] font-black uppercase tracking-widest">{item.category}</span>
                  </div>
